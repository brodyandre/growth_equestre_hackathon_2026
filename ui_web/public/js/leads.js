...
const PATH = "/leads";
const LEADS_FETCH_PAGE_SIZE = 500;
const LEADS_FETCH_MAX_PAGES = 200;
...
function updateFilteredExportUi() {
  ...
  const serverTotal =
    Number.isFinite(Number(state.serverTotal)) && Number(state.serverTotal) >= 0
      ? Number(state.serverTotal)
      : Array.isArray(state.viewRows)
        ? state.viewRows.length
        : 0;
  if ($btnExportFilteredCsv) $btnExportFilteredCsv.disabled = total <= 0;
  if ($filteredCountInfo) {
    $filteredCountInfo.textContent = `${total} lead(s) filtrado(s) de ${serverTotal} total`;
  }
}
...
function leadOptionLabel(lead) {
  const registro = Number(lead?.row_num);
  const registroLabel = Number.isInteger(registro) && registro > 0 ? String(registro) : "-";
  const cityUf = `${lead.cidade}/${lead.uf}`;
  const score = formatScoreValue(lead.score);
  return `${registroLabel} | ${lead.nome} • ${lead.segmento} • ${cityUf} • ${lead.status} • score=${score} • ${shortId(lead.id)}...`;
}
...
async function handleDeleteLead() {
  ...
  if (checkedIds.length) {
    ...
    try {
      ...
      state.serverTotal = state.rawLeads.length;
...
function buildLeadsListUrl(limit, offset) {
  const query = new URLSearchParams();
  query.set("limit", String(Math.max(1, Math.floor(Number(limit) || LEADS_FETCH_PAGE_SIZE))));
  query.set("offset", String(Math.max(0, Math.floor(Number(offset) || 0))));
...
async function fetchLeadsCount() {
  try {
    const payload = await requestJson(`/api${PATH}/count`, { cache: "no-store" });
...
async function fetchAllLeadsDynamic(expectedTotal) {
  const rows = [];
  const total = Number.isFinite(expectedTotal) && expectedTotal >= 0 ? Number(expectedTotal) : null;
  const pageSize = LEADS_FETCH_PAGE_SIZE;
  let pages = 0;

  if (total !== null) {
    for (let offset = 0; offset < total && pages < LEADS_FETCH_MAX_PAGES; offset += pageSize) {
      const batch = await requestJson(buildLeadsListUrl(pageSize, offset), {
        cache: "no-store",
        timeoutMs: 45000,
      });
    ...
    return dedupeLeadsById(rows);
  }

  let offset = 0;
  while (pages < LEADS_FETCH_MAX_PAGES) {
    const batch = await requestJson(buildLeadsListUrl(pageSize, offset), {
      cache: "no-store",
      timeoutMs: 45000,
    });
...
async function load() {
  ...
  try {
    ...
    const data = await fetchAllLeadsDynamic(expectedTotal);
    state.rawLeads = Array.isArray(data) ? data : [];
    state.serverTotal = Number.isFinite(expectedTotal)
      ? Math.max(Number(expectedTotal), state.rawLeads.length)
      : state.rawLeads.length;
