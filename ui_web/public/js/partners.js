...
const PATH = "/partners";
const PARTNERS_FETCH_PAGE_SIZE = 500;
const PARTNERS_FETCH_MAX_PAGES = 200;
...
function buildPartnersListUrl(limit, offset) {
  const query = new URLSearchParams();
  query.set("limit", String(Math.max(1, Math.floor(Number(limit) || PARTNERS_FETCH_PAGE_SIZE))));
...
async function fetchPartnersCount() {
  try {
    const payload = await fetch(`/api${PATH}/count`, { cache: "no-store" });
...
async function fetchAllPartnersDynamic(expectedTotal) {
  const rows = [];
  const total = Number.isFinite(expectedTotal) && expectedTotal >= 0 ? Number(expectedTotal) : null;
  const pageSize = PARTNERS_FETCH_PAGE_SIZE;
  let pages = 0;

  if (total !== null) {
    for (let offset = 0; offset < total && pages < PARTNERS_FETCH_MAX_PAGES; offset += pageSize) {
      const resp = await fetch(buildPartnersListUrl(pageSize, offset), { cache: "no-store" });
  ...
  let offset = 0;
  while (pages < PARTNERS_FETCH_MAX_PAGES) {
    const resp = await fetch(buildPartnersListUrl(pageSize, offset), { cache: "no-store" });
    if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
    const batch = await resp.json();
    const items = Array.isArray(batch) ? batch : [];
...
function dedupePartnerRows(rows) {
  const out = [];
  const seen = new Set();
  for (const row of Array.isArray(rows) ? rows : []) {
    const key = normalizeText(row?.id) || `${normalizeText(row?.cnpj)}|${normalizeText(row?.nome_fantasia)}`;
...
async function load() {
  try {
    const expectedTotal = await fetchPartnersCount();
    const rows = await fetchAllPartnersDynamic(expectedTotal);
    state.allRows = sortPartners(dedupePartnerRows(rows.map(mapPartner)));
  } catch (_err) {
    state.allRows = [
      {
        id: "demo-partner-1",
