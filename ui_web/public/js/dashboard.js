...
function renderOverviewPartnerKpis(summaryList) {
  ...
  const SEGMENT_ORDER = ["CAVALOS", "SERVICOS", "EVENTOS", "EQUIPAMENTOS"];
  const ordered = [...summaryList].sort((a, b) => {
    const totalDiff = Number(b.total || 0) - Number(a.total || 0);
    if (totalDiff !== 0) return totalDiff;

    const aSeg = String(a.segmento || "").toUpperCase();
    const bSeg = String(b.segmento || "").toUpperCase();
    const aRank = SEGMENT_ORDER.indexOf(aSeg);
    const bRank = SEGMENT_ORDER.indexOf(bSeg);
    const safeARank = aRank >= 0 ? aRank : Number.MAX_SAFE_INTEGER;
    const safeBRank = bRank >= 0 ? bRank : Number.MAX_SAFE_INTEGER;
  ...
  const cards = [
    ...
    ...ordered.map((row) => ({ label: segmentLabel(row.segmento), total: row.total })),
  ];
