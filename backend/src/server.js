...
/**
 * Handler padrÃ£o para evitar crash e responder com erro consistente.
 */
...
function parseBooleanFlag(value, fallback = false) {
  ...
  if (["0", "false", "no", "nao", "nÃ£o", "n", "off"].includes(raw)) return false;
  return fallback;
}
...
async function handleLeadUpdate(req, res) {
  ...
  const currentLeadResult = await query(
    `SELECT id, score, status, crm_stage, score_motivos, score_engine,
            score_model_name, score_probability, score_scored_at, score_meta
       FROM leads
      WHERE id = $1`,
    [leadId]
  );
  if (!currentLeadResult.rows.length) {
  ...
  sets.push(`score = $${i++}`);
  values.push(currentLead.score);
  sets.push(`status = $${i++}`);
  values.push(currentLead.status);
  sets.push(`crm_stage = $${i++}`);
  values.push(currentLead.crm_stage);
  sets.push(`score_motivos = $${i++}`);
  values.push(currentLead.score_motivos === null ? null : JSON.stringify(currentLead.score_motivos));
  sets.push(`score_engine = $${i++}`);
  values.push(currentLead.score_engine);
  sets.push(`score_model_name = $${i++}`);
  values.push(currentLead.score_model_name);
  sets.push(`score_probability = $${i++}`);
  values.push(currentLead.score_probability);
  sets.push(`score_scored_at = $${i++}`);
  values.push(currentLead.score_scored_at);
  sets.push(`score_meta = $${i++}`);
  values.push(currentLead.score_meta === null ? null : JSON.stringify(currentLead.score_meta));
  values.push(leadId);
...
async function ensureCrmSchema() {
  // CRM extras used by the Admin UI (Kanban + PrÃ³xima aÃ§Ã£o)
  await query(`
    ALTER TABLE leads
  ...
  // Notes/audit trail for CRM actions (optional, but used by the UI for "PrÃ³xima aÃ§Ã£o")
  await query(`
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!nome || !segmento_interesse) {
      return res
        .status(400)
        .json({ error: "Campos obrigatÃ³rios: nome, segmento_interesse" });
    }
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!lead_id || !event_type) {
      return res
        .status(400)
        .json({ error: "Campos obrigatÃ³rios: lead_id, event_type" });
    }
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (leadR.rows.length === 0) return res.status(404).json({ error: "Lead nÃ£o encontrado" });
...
/**
 * CRM (Kanban): board + mover estÃ¡gio + prÃ³xima aÃ§Ã£o + matching de parceiros
...
function resolveBoardStage(lead) {
  ...
  // Se o CRM ainda estÃ¡ no default INBOX, deixa o status comercial conduzir as colunas.
...
async function handleCrmMove(req, res) {
  ...
  if (!leadId) return res.status(400).json({ error: "lead_id Ã© obrigatÃ³rio" });
  ...
  if (!leadR.rows.length) return res.status(404).json({ error: "Lead nÃ£o encontrado" });
...
async function handleCrmAddNote(req, res) {
  ...
  if (!note) return res.status(400).json({ error: "note Ã© obrigatÃ³rio" });
  ...
  // Se for uma prÃ³xima aÃ§Ã£o, tambÃ©m atualiza o snapshot no lead (usado no card do Kanban)
  if (next) {
    await query(
      `UPDATE leads
...
async function handleCrmMatches(req, res) {
  ...
  if (!leadRes.rows.length) return res.status(404).json({ error: "Lead nÃ£o encontrado" });
  ...
  const items = pRes.rows
    .map((p) => {
      ...
      // HeurÃ­stica de score simples e explicÃ¡vel
      ...
      if (pr === "ALTA") score += 2;
      else if (pr === "MEDIA" || pr === "MÃ‰DIA") score += 1;
...
// Mantido apÃ³s as rotas principais para preservar os contratos jÃ¡ utilizados pela UI.
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!lead_id) return res.status(400).json({ error: "lead_id Ã© obrigatÃ³rio" });
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const expectedLeadsRaw = body.expected_leads ?? body.expectedLeads ?? req.query?.expected_leads;
    const expectedLeadsParsed = Number.parseInt(String(expectedLeadsRaw ?? ""), 10);
    const expectedLeads =
      Number.isFinite(expectedLeadsParsed) && expectedLeadsParsed > 0 ? expectedLeadsParsed : null;

    const randomStateRaw = body.random_state ?? body.randomState ?? req.query?.random_state;
    const randomStateParsed = Number.parseInt(String(randomStateRaw ?? "42"), 10);
    const randomState = Number.isFinite(randomStateParsed)
      ? Math.max(1, Math.min(randomStateParsed, 2147483647))
      : 42;

    const minRowsRaw = body.min_rows ?? body.minRows ?? req.query?.min_rows;
    const minRowsParsed = Number.parseInt(String(minRowsRaw ?? "200"), 10);
    const minRows = Number.isFinite(minRowsParsed) ? Math.max(50, Math.min(minRowsParsed, 500000)) : 200;

    const ignoreExpectedMismatch = parseBooleanFlag(
      body.ignore_expected_mismatch ?? body.ignoreExpectedMismatch ?? req.query?.ignore_expected_mismatch,
      false
    );

    const searchModeRaw = String(body.search_mode ?? body.searchMode ?? req.query?.search_mode ?? "quick")
      .trim()
      .toLowerCase();
    const searchMode = searchModeRaw === "full" ? "full" : "quick";
    ...
    const scoringResp = await fetch(`${SCORING_URL}/admin/retrain`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });

    const raw = await scoringResp.text();
    let parsed = null;
    try {
    ...
    if (!scoringResp.ok) {
      const details =
        parsed?.detail || parsed?.error || parsed?.message || raw || `HTTP ${scoringResp.status}`;
      return res.status(scoringResp.status).json({
        error: "Falha ao retreinar modelo no scoring_service",
        details: String(details),
        payload,
      });
    }

    return res.json(parsed || { ok: true, payload });
  })
);

// -----------------------------
// DEMO â€” SEED / RESET PARTNERS
// -----------------------------
 ...
 *  { "n": 160, "replace": false }
 *
 * - n: quantidade desejada (padrÃ£o 160, mÃ¡ximo 1000)
...
/**
 * POST /demo/reset
 * Limpa dados de demo para reiniciar o fluxo completo da apresentaÃ§Ã£o.
...
app.post(
  ...
  asyncHandler(async (_req, res) => {
    ...
    res.json({
      ...
      message: "Parceiros removidos (reset concluÃ­do).",
    });
  })
);
