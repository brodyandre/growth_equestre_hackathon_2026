...
Também inclui automação por evento no painel de detalhes:
- campo **Evento objetivo (ajusta o score e move o lead)**;
- botão **Aplicar evento** para registrar o evento e recalcular score/status/etapa automaticamente.

Regras operacionais atuais do Kanban:
- faixas de score por etapa: `CURIOSO (0-39)`, `AQUECENDO (40-69)`, `QUALIFICADO (70-100)`;
- para subir para `QUALIFICADO`, além do score, é obrigatório cumprir o gate de sinais:
  `budget_confirmed + timeline_confirmed + need_confirmed`;
- se faltar algum sinal, o score continua subindo/descendo normalmente, mas o lead permanece em `AQUECENDO`;
- movimentação manual de etapa (`/crm/move`) ajusta o score para a faixa da coluna de destino, mantendo consistência visual e operacional.
